...
import { getCloudinaryUrl, getActiveAccessKey, type CloudAccount } from './provision.js';
...
export function printHumanSummary(account: CloudAccount, ctx: OutputContext): void {
  ...
  console.log();
  console.log(pc.green(pc.bold('  ✔ Cloud provisioned — your media is live on Cloudinary')));
  console.log();
  ...
  console.log(`  ${pc.dim('makes the cloud permanent with the same credentials.')}`);
  console.log();
...
export function printPlainSummary(account: CloudAccount, ctx: OutputContext): void {
  const env = account.product_environments[0];
