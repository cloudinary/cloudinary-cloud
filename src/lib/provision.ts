/**
 * Client for the Cloudinary sandbox provisioning endpoint.
 *
 * Contract: POST /v1_1/provisioning/sandboxes (public, unauthenticated,
 * rate limited per IP).
 */
...
export const SANDBOXES_PATH = '/v1_1/provisioning/sandboxes';
...
export async function provisionSandbox(
  ...
  let res: Response;
  try {
    res = await doFetch(`${host}${SANDBOXES_PATH}`, {
      method: 'POST',
  ...
  return (await res.json()) as SandboxAccount;
}

/** First enabled access key (falling back to the first key) — the credentials the CLI surfaces. */
export function getActiveAccessKey(env: ProductEnvironment): ApiAccessKey {
  const key = env.api_access_keys.find(k => k.enabled !== false) ?? env.api_access_keys[0];
  if (!key) throw new ProvisionError('Response contained no API access keys.', 0);
  return key;
}
